...
describe Puppet::Type.type(:rcsrepo) do
  ...
  it 'should not accept revisions that have whitespace characters' do
    expect { subject[:revision] = ' aoeu '}.should raise_error(Puppet::Error, /Invalid value/)
  end
  it 'should require that path be absolute' do
    expect { Puppet::Type.type(:rcsrepo).new(:path => 'foo') }.to raise_error(Puppet::Error, /Path must be absolute/)
  end
  it 'should fail for invalid URIs' do
    expect {subject[:source] = 'mywebsite com'}.should raise_error(Puppet::Error, /bad URI/)
  end
